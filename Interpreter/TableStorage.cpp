...
namespace dust {
	namespace impl {
		...
		size_t expand(std::vector<std::pair<impl::Table*, int>>& vec) {
			vec.push_back(std::make_pair(nullptr, 0));
			return vec.size() - 1;
		}

		size_t TableStorage::nxt_record(impl::Table* t) {
			size_t alloc = open.empty() ? expand(records) : pop();
			records[alloc] = std::make_pair(t, 0);
			return alloc;
		}

		size_t TableStorage::loadRef(impl::Table* t) {
		...
		impl::Table* TableStorage::deref(size_t idx) {
