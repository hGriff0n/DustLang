	...
	namespace test {
		void run_tests(EvalState& e) {
			...
			t.init_sub_test("Tricky Operations");
				...
				// Throws an exception here. Doesn't throw an exception in repl
				t.require_eval("a, b: 3, (c: 4)", 4);
				t.require_true("a = 3");

				t.require_eval("3 +-3", 0);

				// 97 would previously give a "Passing" result even though Operator evaluated the expression wrong
					// This was because a already equaled 3 coming into this test, rendering the order of execution moot
				t.eval("a:2");
